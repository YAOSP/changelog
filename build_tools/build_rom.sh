...
AOSP_TARGET_PACKAGE="out/target/product/$1/AOSP-$1-`grep ro.build.version.release= out/target/product/$1/system/build.prop | awk -F= '{print $2;}'`-`grep ro.build.id= out/target/product/$1/system/build.prop | awk -F= '{print $2;}'`-`date +'%Y%m%d'`.zip"
...
if [ -e out/dist/aosp_$1-ota-eng.$USER.zip ]; then
   ...
   echo
   echo "Package : $AOSP_TARGET_PACKAGE"
   echo
